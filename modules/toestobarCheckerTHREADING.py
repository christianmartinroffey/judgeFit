...
def main():
    ...
    pose_processor = pm.PoseProcessor(detector)
    ...
    processing_thread = threading.Thread(
        target=pose_processor.process_frame,
    ...
    while True:
        ...
        if not pose_processor.result_queue.empty():
            processed_frame, count, no_rep, outcome, angle, full_range, full_extension, is_movement_started = pose_processor.result_queue.get()
            print("Processed frame retrieved.")
    ...
    #             break
    #
    #         pose_processor.frame_queue.put(img)
    #         if not pose_processor.result_queue.empty():
    #
    #             processed_frame, count, no_rep, outcome, angle, full_range, full_extension, is_movement_started = result_queue.get()
    #             # Display logic
    #             cv2.putText(processed_frame, f'Reps: {count}', (50, 100), cv2.FONT_HERSHEY_PLAIN, 3, (255, 0, 0), 5)
    #             cv2.putText(processed_frame, f'No Reps: {no_rep}', (500, 100), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255),
    #                         5)
    #             cv2.putText(processed_frame, f'Outcome: {outcome}', (50, 200), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255),
    #                         5)
    #
    #             cv2.imshow("Image", processed_frame)
    #             if cv2.waitKey(1) & 0xFF == ord('q'):
    ...
    #
    # finally:
    #     pose_processor.frame_queue.put(None)
